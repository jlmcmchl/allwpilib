 ...
 * 
 * A motion profile generates a trajectory that smoothly transitions from one state 
 * to another while respecting constraints such as maximum velocity and 
 ...
 using Distance_t = units::unit_t<Distance>;
  ...
  class Curve {
   public:
     ...
     */
    Distance_t ComputeDistanceFromVelocity(Velocity_t velocity) const;
     ...
     */
    units::second_t TimeToState(const State& goal) const;
     ...
     */
    Velocity_t ComputeVelocityFromTime(units::second_t t) const;
     ...
     */
    Distance_t ComputeDistanceFromTime(units::second_t t) const;
     ...
     */
    Velocity_t IntersectionVelocity(const Curve& other) const;
  ...
  /**
   * Abstract base class for motion constraints that determine the allowable motion
   * characteristics.
   *
  ...
  class Constraints {
   public:
     ...
     */
    T ThroughState(const State& state, bool direction) const;
  ...
  /**
   * Determines whether the input direction should be flipped based on the current
   * and goal states.
   *
   * @param current The current state (position and velocity).
   * @param goal The desired goal state (position and velocity).
