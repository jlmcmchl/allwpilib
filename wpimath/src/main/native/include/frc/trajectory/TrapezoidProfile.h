...
#include "FullStateMotionProfile.h"
#include "VelocityMotionProfile.h"
 ...
 public:
  ...
  class Curve : public MotionProfile<Distance>::Curve {
   public:
     ...
     */
    Curve(const Constraints& constraints, const typename MotionProfile<Distance>::State& state,
          Acceleration_t acceleration);

    Distance_t ComputeDistanceFromVelocity(Velocity_t velocity) const override;
    units::second_t TimeToState(const typename MotionProfile<Distance>::State& goal) const override;
    ...
    Velocity_t IntersectionVelocity(const typename MotionProfile<Distance>::Curve& other) const override;
   ...
   */
  class Constraints : public MotionProfile<Distance>::template Constraints<Curve> {
   public:
     ...
     */
    Constraints(Velocity_t maxVelocity, Acceleration_t acceleration)
     ...
     * @param direction Whether to move in the negative (true) or positive (false) d`irection.
     * @return A new curve passing through the given state.
     */
    Curve ThroughState(const typename MotionProfile<Distance>::State& state, bool direction) const {
        return Curve(*this, state, direction ? -maxAcceleration : maxAcceleration);
    };
  };
  ...
  static FullStateMotionProfile<Distance> FullState(const Constraints& constraints) {
    return FullStateMotionProfile<Distance>(constraints, constraints);
  }
  ...
  static FullStateMotionProfile<Distance> FullState(const Constraints& forwardConstraints,
    ...
    return FullStateMotionProfile<Distance>(forwardConstraints, reverseConstraints);
  }
  ...
  static VelocityMotionProfile<Distance> VelocityOnly(const Constraints& constraints) {
        return VelocityMotionProfile<Distance>(constraints, constraints);
  }
  ...
  static VelocityMotionProfile<Distance> VelocityOnly(const Constraints& forwardConstraints,
                                          const Constraints& reverseConstraints) {
        return VelocityMotionProfile<Distance>(forwardConstraints, reverseConstraints);
  }
};
