...
import edu.wpi.first.math.kinematics.Copy;
import edu.wpi.first.math.kinematics.Difference;
import edu.wpi.first.math.kinematics.Odometry;
import edu.wpi.first.math.kinematics.PositionKinematics;
...
public class PoseEstimator<
    DInput extends Difference<DInput> & Copy<DInput> & Interpolatable<DInput>> {
  private final PositionKinematics<DInput> m_kinematics;
  private final Odometry<DInput> m_odometry;
  ...
  public PoseEstimator(
    ...
    m_odometry =
        new Odometry<DInput>(kinematics, gyroAngle, distanceMeasurements, initialPoseMeters);
   ...
   */
  public void resetPosition(Rotation2d gyroAngle, DInput distanceMeasurements, Pose2d poseMeters) {
    // Reset state estimate and error covariance
    m_odometry.resetPosition(gyroAngle, distanceMeasurements, poseMeters);
    m_poseBuffer.clear();
  }
  ...
  public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
    ...
    m_odometry.resetPosition(
        sample.get().gyroAngle, sample.get().distances, sample.get().poseMeters.exp(scaledTwist));

    // Step 6: Record the current pose to allow multiple measurements from the same timestamp
    m_poseBuffer.addSample(
        timestampSeconds,
        new InterpolationRecord(
            getEstimatedPosition(), sample.get().gyroAngle, sample.get().distances));
    ...
    for (Map.Entry<Double, InterpolationRecord> entry :
      ...
      updateWithTime(entry.getKey(), entry.getValue().gyroAngle, entry.getValue().distances);
    }
  }
   ...
   */
  public Pose2d update(Rotation2d gyroAngle, DInput distances) {
    return updateWithTime(WPIUtilJNI.now() * 1.0e-6, gyroAngle, distances);
  }
   ...
   * @return The estimated pose of the robot in meters.
   */
  public Pose2d updateWithTime(double currentTimeSeconds, Rotation2d gyroAngle, DInput distances) {
    m_odometry.update(gyroAngle, distances);
    m_poseBuffer.addSample(
        currentTimeSeconds, new InterpolationRecord(getEstimatedPosition(), gyroAngle, distances));
  ...
  private class InterpolationRecord implements Interpolatable<InterpolationRecord> {
    ...
    // The distance measured by the drivetrain.
    private final DInput distances;
     ...
     */
    private InterpolationRecord(Pose2d poseMeters, Rotation2d gyro, DInput distances) {
      this.poseMeters = poseMeters;
      this.gyroAngle = gyro;
      this.distances = distances.copy();
    }
    ...
    public InterpolationRecord interpolate(InterpolationRecord endValue, double t) {
      if (t < 0) {
        ...
        var distance_lerp = this.distances.interpolate(endValue.distances, t);
        ...
        Twist2d twist = m_kinematics.toTwist2d(distance_lerp.minus(distances));
        twist.dtheta = gyro_lerp.minus(gyroAngle).getRadians();

        return new InterpolationRecord(poseMeters.exp(twist), gyro_lerp, distance_lerp);
      }
    }
    ...
    public boolean equals(Object obj) {
      ...
      try {
        ...
        && Objects.equals(distances, record.distances)
    ...
    @Override
    public int hashCode() {
      return Objects.hash(gyroAngle, distances, poseMeters);
    }
  }
}
