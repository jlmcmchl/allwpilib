...
#include "frc/trajectory2/TrapezoidProfile.h"  // NOLINT(build/include_order)
...
TEST(TrapezoidProfileTest, ReachesGoal) {
  frc::TrapezoidProfile<units::meter>::Constraints constraints{1.75_mps,
                                                               0.75_mps_sq};
  frc::MotionProfile<units::meter>::State goal{3_m, 0_mps};
  frc::MotionProfile<units::meter>::State state;

  auto profile = frc::TrapezoidProfile<units::meter>::FullState(constraints);
  for (int i = 0; i < 450; ++i) {
